...
/// Get FreeBSD `Processes`
pub(crate) fn processes(
    artifact: &ProcessOptions,
    output: &mut Output,
    filter: &bool,
) -> Result<(), FreeBSDArtifactError> {
    ...
    let proc_data = match results {
        Ok(data) => data,
        Err(err) => {
            warn!("[artemis-core] Artemis FreeBSD failed to get process list: {err:?}");
            return Err(FreeBSDArtifactError::Process);
        }
    };
    ...
    let serde_data = match serde_data_result {
        ...
        Err(err) => {
            ...
            return Err(FreeBSDArtifactError::Serialize);
        }
    };
...
/// Get FreeBSD `Systeminfo`
pub(crate) fn systeminfo(output: &mut Output, filter: &bool) -> Result<(), FreeBSDArtifactError> {
    let start_time = time::time_now();
...
pub(crate) fn files(
    ...
    match artifact_result {
        Ok(_) => {}
        Err(err) => {
            error!("[artemis-core] Artemis FreeBSD failed to get file listing: {err:?}");
            return Err(FreeBSDArtifactError::File);
        }
    };
    Ok(())
}

/// Output FreeBSD artifacts
pub(crate) fn output_data(
        ...
        if let Some(script) = &output.filter_script.clone() {
            ...
            if let Some(name) = &output.filter_name.clone() {
                ...
                return match filter_result {
                    ...
                    Err(err) => {
                        error!(
                            "[artemis-core] Could not apply filter script to FreeBSD data: {err:?}"
                        );
                        Err(FreeBSDArtifactError::FilterOutput)
                    }
                };
            }
            ...
            return match filter_result {
                ...
                Err(err) => {
                    error!(
                    "[artemis-core] Could not apply unknown filter script to FreeBSD data: {err:?}"
                );
                    Err(FreeBSDArtifactError::FilterOutput)
                }
            };
    ...
    let output_status = if output.format == "json" {
        ...
        return Err(FreeBSDArtifactError::Format);
    };
    match output_status {
        Ok(_) => {}
        Err(err) => {
            error!("[artemis-core] Could not output data: {err:?}");
            return Err(FreeBSDArtifactError::Output);
        }
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use crate::artifacts::os::freebsd::artifacts::{files, output_data, processes, systeminfo};
    use crate::structs::artifacts::os::files::FileOptions;
    use crate::structs::artifacts::os::processes::ProcessOptions;
    use crate::utils::artemis_toml::Output;
    use crate::utils::time;
