...
impl TableDump {
    ...
    pub(crate) fn dump_table<'a>(
        ...
        for entry in catalog {
                ...
                && entry.catalog_type == CatalogType::Column
            {
                let column_info = ColumnInfo {
                    column_type: TableDump::get_column_type(&entry.column_or_father_data_page),
                    column_name: entry.name,
                    column_data: Vec::new(),
                    column_id: entry.id,
                    column_flags: TableDump::get_column_flags(&entry.flags),
        ...
        for tag in table_page_data.page_tags {
            ...
            if table_page_data.page_flags.contains(&PageFlags::Leaf) {
                ...
                TableDump::parse_row(leaf_row, &mut info.column_info);
    ...
    fn create_table_data(
        ...
        for column_row in column_rows {
            ...
            for column in column_row {
                ...
                let result = TableDump::column_data_to_string(
                    &dump.column_type,
                    &column.column_data,
                    &column.column_flags,
    ...
    pub(crate) fn parse_row(leaf_row: PageLeaf, column_info: &mut [ColumnInfo]) {
        ...
        let _ = TableDump::parse_fixed_data(
            &leaf_data.last_fixed_data,
            &leaf_data.fixed_data,
            column_info,
        );

        let _ = TableDump::parse_variable_data(
        ...
        while &column <= last_fixed_data {
            for (_, entry) in column_info.iter_mut().enumerate() {
                if entry.column_id == column as i32 {
                    let (input, column_data) = TableDump::nom_fixed_column(
                        &entry.column_type,
                        data,
                        entry.column_space_usage,
                    )?;
                    data = input;
    ...
    fn parse_variable_data<'a>(
        ...
        if !data.is_empty() {
            TableDump::parse_tagged_data(data, column_info)?;
        }
        Ok((data, ()))
    }
    ...
    fn nom_fixed_column<'a>(
        ...
        let (input, column_data) = match column_type {
            ...
            | ColumnType::Text
            | ColumnType::LongText => take(column_space_usage as u32)(data)?,
            _ => {
...
#[cfg(test)]
mod tests {
    use super::{ColumnInfo, TableDump};
    use crate::{
        artifacts::os::windows::ese::{
    ...
    fn test_parse_fixed_data() {
        ...
        let (_, _) = TableDump::parse_fixed_data(&last_fixed, &test, &mut info_vec).unwrap();
    ...
    fn test_parse_variable_data() {
        ...
        let mut info_vec = vec![info];
        let (_, _) = TableDump::parse_variable_data(&last_variable, &test, &mut info_vec).unwrap();
        assert_eq!(
    ...
    fn test_parse_tagged_data() {
        ...
        let mut info_vec = vec![info];
        let (_, _) = TableDump::parse_tagged_data(&test, &mut info_vec).unwrap();
        assert_eq!(
            info_vec[0].column_data,
    ...
    fn test_column_data_to_string() {
        ...
        let (_, results) =
            TableDump::column_data_to_string(&col_type, &data, &flags, &tagged_flags).unwrap();
        assert_eq!(results, "4");
    }
    ...
    fn test_create_table_data() {
        ...
        let rows = vec![info];
        let name = "test";
        let results = TableDump::create_table_data(&rows, name);
    ...
    fn test_parse_row() {
        ...
        let _ = TableDump::parse_row(leaf, &mut col);
        assert_eq!(
            col[0].column_data,
    ...
    fn test_clear_column_data() {
        ...
        TableDump::clear_column_data(&mut info_vec);
    ...
    fn test_get_column_type() {
        let test = 2;
        let result = TableDump::get_column_type(&test);
    ...
    fn test_nom_fixed_column() {
        ...
        let (_, result) = TableDump::nom_fixed_column(&data_type, &test_data, 0).unwrap();
    ...
    fn test_get_column_flags() {
        let test = 4096;
        let flags = TableDump::get_column_flags(&test);
    ...
    fn test_get_decompressed_data_seven_bit() {
        ...
        let (_, results) = TableDump::get_decompressed_data(&test).unwrap();
    ...
    fn test_get_decompressed_data_huffman() {
        ...
        let (_, result) = TableDump::get_decompressed_data(&test).unwrap();
