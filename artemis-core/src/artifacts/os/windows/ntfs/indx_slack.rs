use super::parser::{CompressionType, RawFilelist};
use crate::{
...
impl RawFilelist {
    ...
    pub(crate) fn get_indx(
        ...
        while let Some(attribute) = attributes.next(fs) {
            ...
            if attr_type == "IndexRoot" {
                file_info.append(&mut RawFilelist::get_slack(
                    fs,
    ...
    fn get_slack(
        ...
        while let Some(attribute) = attributes.next(fs) {
            ...
            let slack_results = RawFilelist::parse_indx_slack(&temp_buff, directory, depth);
            match slack_results {
    ...
    fn parse_indx_slack<'a>(
        ...
        while !indx_data.is_empty() && indx_data.len() > min_parent_size {
            let (_, (mft_parent_reference, record_size, allocated_size)) =
                RawFilelist::get_mft_parent_reference(indx_data)?;
            ...
            while !indx_slack_data.is_empty() {
                ...
                let slack_found = RawFilelist::search_slack(indx_slack_data, mft_parent_reference);
                ...
                let inode = if data.len() > mft_entry_start {
                    let mft_entry =
                        &data[data.len() - mft_entry_start..data.len() - mft_entry_size];
...
mod tests {
    use crate::{
        artifacts::os::windows::ntfs::parser::RawFilelist,
        filesystem::ntfs::sector_reader::SectorReader,
    };
    use ntfs::Ntfs;
    use std::{
    ...
    fn test_search_slack() {
        ...
        let (search_hit, nomed_data) = RawFilelist::search_slack(&test_data, &search_data).unwrap();
    ...
    fn test_get_mft_parent_reference() {
        ...
        let (_, (result, record_size, size)) =
            RawFilelist::get_mft_parent_reference(&test_data).unwrap();
    ...
    fn test_parse_indx_slack() {
        ...
        let (_, result) = RawFilelist::parse_indx_slack(&buffer, &directory, &depth).unwrap();
    ...
    fn test_get_indx() {
        ...
        let result = RawFilelist::get_indx(&mut fs, &root_dir, "test", &1);
        assert!(result.len() > 0);
    }
