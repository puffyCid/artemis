...
use crate::{
    artifacts::os::{systeminfo::info::SystemInfo, windows::shimcache::error::ShimcacheError},
...
pub(crate) struct ShimcacheEntry {
    pub(crate) entry: u32,
...
impl ShimcacheEntry {
    pub(crate) fn parse_shimdata(
        ...
        let os = SystemInfo::get_os_version();
...
mod tests {
    ...
    fn test_parse_shimdata() {
        ...
        for entry in result {
            let shim_data =
                ShimcacheEntry::parse_shimdata(&entry.shim_data, &entry.key_path).unwrap();
    ...
    fn test_parse_shimdata_win11() {
        ...
        let path = "test";
        let shim_data = ShimcacheEntry::parse_shimdata(test_data, path).unwrap();
