...
use crate::{
    artifacts::os::windows::wmi::class::{extract_cim_data, get_parent_props, parse_qualifier},
    utils::{
        nom_helper::{
...
pub(crate) fn parse_instances<'a>(
    ...
    for instance in instances {
        for classes in &mut *classes_vec {
            ...
            let name_option = classes.get_mut(&hash_name);
            ...
            for class in class_entries.iter_mut() {
                if hash_name == class.class_hash {
                    if !class.super_class_name.is_empty() {
                        // Need to get parent properties too
                        let mut props =
                            get_parent_props(&class.super_class_name, indexes, objects_data, pages);
            ...
            //println!("full data: {:?}", instance.data);
            let (remaining, prop_bit_data) = parse_instance_props(&instance.data, &prop_count)?;

            // Now update property instanceValue
            for class in class_entries.iter_mut() {
                for prop in class.properties.iter_mut() {
                    let value = check_value(&prop_bit_data, &(prop.property_index as usize));
                    prop.instance_value = value;
                }
            }
...
fn get_prop_data_size(classes: &[ClassInfo]) -> u32 {
    ...
    let two_bytes = vec![CimType::Uint16, CimType::Sint16];
