...
pub(crate) fn list_files_directories(path: &str) -> Result<Vec<String>, FileSystemError> {
    ...
    let dir = match dir_result {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Failed to get directory contents: {err:?}");
    ...
    for entry_result in dir {
        let entry = match entry_result {
            Ok(result) => result,
            Err(err) => {
                error!("[artemis-core] Failed to get directory entry: {err:?}");
                continue;
            }
        };
...
pub(crate) fn hash_file(hashes: &Hashes, path: &str) -> (String, String, String) {
    ...
    let mut file = match file_open {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Failed to hash file {path}: {err:?}");
            return (md5_string, sha1_string, sha256_string);
        }
    };
    ...
    loop {
        ...
        let bytes = match bytes_result {
            Ok(result) => result,
            Err(err) => {
                error!("[artemis-core] Failed to read file: {err:?}");
                return (md5_string, sha1_string, sha256_string);
            }
        };
        let finished = 0;
...
pub(crate) fn get_file_size(path: &str) -> u64 {
    ...
    match meta {
        ...
        Err(err) => {
            error!("[artemis-core] Failed to get file size: {err:?}");
            0
        }
    }
}
