...
pub(crate) fn raw_hash_data(
    ...
    loop {
        ...
        let bytes = match bytes_result {
            Ok(result) => result,
            Err(err) => {
                error!("[artemis-core] Failed to read data for hashing: {err:?}");
                break;
            }
        };
        let finished = 0;
...
pub(crate) fn raw_read_by_file_ref(
    ...
    let ntfs_file = match ntfs_file_result {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Failed to get NTFS file, error: {err:?}");
            return Err(FileSystemError::NotFile);
        }
    };
    let data_name = "";
    ...
    let ntfs_data = match ntfs_data_result {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Failed to get NTFS data error: {err:?}");
    ...
    let mut data_attr_value = match data_result {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Failed to get NTFS attribute data error: {err:?}");
    ...
    let file_data = match file_data_result {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Could not read file error: {err:?}");
...
pub(crate) fn iterate_ntfs(
    ...
    let index = match index_result {
        Ok(result) => result,
        Err(err) => {
            error!("[artemis-core] Failed to get NTFS index directory, error: {err:?}");
    ...
    while let Some(entry) = iter.next(fs) {
        ...
        let entry_index = match entry_result {
            Ok(result) => result,
            Err(err) => {
                error!("[artemis-core] Failed to get NTFS entry index, error: {err:?}");
                continue;
            }
        };
