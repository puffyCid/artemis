...
pub(crate) async fn save_collection(
    ...
    let collect_path = format!("{storage_path}/collections.jsonl");
    ...
    if !is_file(&collect_path) {
        let status = write_file(&bytes, &collect_path).await;
        if status.is_err() {
            error!(
    ...
    let _ = append_file(&bytes, &collect_path).await;
        ...
        let data = CollectionRequest {
            ...
            info: CollectionInfo {
                ...
                collection: String::from(
                    "",
                ),
            },
        };
