...
const WOF_MAX_SIZE: u64 = 2 * 1024 * 1024 * 1024;

#[derive(Debug, Clone, Copy, PartialEq)]
enum WofCompressionUnit {
    XpressHuffman4k = 4096,
    Lzx32k = 32768,
    XpressHuffman8k = 8192,
    XpressHuffman16k = 16384,
}
...
impl WofCompressionUnit {
    ...
    fn from_method(method: u32) -> AccessorResult<Self> {
        match method {
            ...
            1 => Ok(Self::Lzx32k),
            2 => Ok(Self::XpressHuffman8k),
            3 => Ok(Self::XpressHuffman16k),
            _ => Err(wof_err(format!(
                "unsupported WOF compression method: {method}"
            ))),
        }
    }
...
pub(crate) fn decompress_wof<T: Read + Seek>(
    ...
    if unit == WofCompressionUnit::Lzx32k {
    ...
    if uncompressed_size == 0 {
        return Err(wof_err("WOF file has zero logical uncompressed size"));
    }

    let array_len = uncompressed_size as u64 / unit.as_u32() as u64;
    let (_, decompressed) = walk_offset_table(
        &compressed_blob,
        array_len,
        unit.as_u32(),
        uncompressed_size,
    ...
    if decompressed.len() != uncompressed_size {
        return Err(wof_err(format!(
            "WOF decompressed size mismatch: expected {uncompressed_size}, got {}",
            decompressed.len()
        )));
    }
    Ok(decompressed)
...
/// Walk offset table and decompress XPRESS Huffman chunks.
fn walk_offset_table(
    ...
    while array_count < array_len {
        let (remaining_input, offset) = if uncompressed_size < LARGE_UNCOMPRESSED_THRESHOLD {
            let (rest, value) = nom_unsigned_four_bytes(input, Endian::Le)?;
            (rest, value as u64)
        } else {
    ...
    while let Some(offset) = offset_iter.next() {
        let (offset_start, first_chunk_data) = take(*offset)(input)?;
        let compressed_data;
        if let Some(next_offset) = offset_iter.peek() {
            let chunk_size = *next_offset - offset;
            let (_, compressed_input) = take(chunk_size)(offset_start)?;
...
/// Decompress the WOF data
fn decompress_chunk(chunk: &[u8], decom_size: u32) -> AccessorResult<Vec<u8>> {
    let mut input = chunk.to_vec();
    let bytes = match decompress_xpress(&mut input, decom_size, &XpressType::XpressHuffman) {
        Ok(result) => result,
        Err(err) => {
...
fn read_attribute_data<T: Read + Seek>(
    reader: &mut T,
    file: &NtfsFile<'_>,
    stream_name: &str,
) -> AccessorResult<Vec<u8>> {
...
fn wof_err(reason: impl Into<String>) -> AccessorError {
    AccessorError::Ntfs {
        path: None,
        reason: reason.into(),
    }
...
/// Handle nom errors to `AccessorError`
fn nom_err<E: std::fmt::Debug>(err: nom::Err<E>) -> AccessorError {
    AccessorError::Ntfs {
        path: None,
        reason: format!("WOF parse error: {err:?}"),
    }
}

#[cfg(test)]
mod tests {
    use super::{parse_reparse, walk_offset_table};
    ...
    fn test_parse_reparse() {
        ...
        assert_eq!(result.sig, 2147483671);
