...
pub(crate) fn parse_loginitems(path: &str) -> Result<Vec<LoginItemsData>, LoginItemError> {
    // Parse PLIST file and get any loginitems data
...
/// Parse `LoginItems` associated with bundled apps. Legacy version
...
/// Get `LoginItems` data from historical embedded `LoginItems` in Apps
