...
pub(crate) fn get_spotlight_meta(paths: &[GlobInfo]) -> Result<SpotlightMeta, SpotlightError> {
    ...
    for path in paths {
        if !path.full_path.contains(".header")
            || path.full_path.contains("dbStr-3")
            || path.full_path.contains(".shadow")
        {
            continue;
        }
        ...
        if path.full_path.contains("dbStr-1") {
            ...
            match prop_results {
                ...
                Err(_err) => {
                    error!("Could not parse dbstr property: {}", path.full_path);
            ...
            match cat_results {
                ...
                Err(_err) => {
                    error!("Could not parse dbstr category: {}", path.full_path);
            ...
            match indexes_results {
                ...
                Err(_err) => {
                    error!("Could not parse dbstr indexes1: {}", path.full_path);
            ...
            match indexes_results {
                ...
                Err(_err) => {
                    error!("Could not parse dbstr indexes2: {}", path.full_path);
...
fn read_dbstr(path: &str) -> Result<Vec<u8>, SpotlightError> {
    ...
    let data = match data_results {
        Ok(results) => results,
        Err(err) => {
            error!("Could not read dbstr file {path}: {err:?}");
