...
use common::unix::{ZshHistory, ZshHistoryData};
...
fn zsh(user_paths: &[String]) -> Result<Vec<ZshHistory>, ShellError> {
    ...
    for path in user_paths {
        ...
        if is_file(&zsh_history) {
            let zsh_data_result = parse_zsh(&zsh_history);
        ...
        if is_directory(&zsh_sessions) {
            let zsh_data_result = parse_zsh_sessions(path, &get_filename(path));
            let mut zsh_data = match zsh_data_result {
                Ok(result) => result,
                Err(err) => {
                    warn!("[shell_history] Could not parse zsh session file at {path}: {err:?}");
                    continue;
                }
            };
...
fn parse_zsh_sessions(
    ...
    for session in sessions {
        ...
        if extension == "history" && is_file(&session) {
            let zsh_data = parse_zsh(&session)?;
            let zsh_history = ZshHistory {
                history: zsh_data,
                path: session,
...
fn parse_zsh(zsh_history: &str) -> Result<Vec<ZshHistoryData>, ShellError> {
    ...
    for line_entry in zsh_iter {
        ...
        let zsh_history_results = parse_line(&zsh_entry, &zsh_regex);
...
#[cfg(test)]
mod tests {
    use super::{get_user_zsh_history, parse_line, parse_zsh, parse_zsh_sessions, zsh};
    ...
    fn test_get_macos_user_zsh_history() {
        let _ = get_user_zsh_history().unwrap();
    }
    ...
    fn test_parse_zsh_sessions() {
        ...
        let results = parse_zsh_sessions(&test_location.display().to_string(), "test").unwrap();
    ...
    fn test_parse_zsh() {
        ...
        let results = parse_zsh(&test_location.display().to_string()).unwrap();
