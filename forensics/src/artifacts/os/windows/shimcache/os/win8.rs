...
pub(crate) fn win8_format<'a>(
    ...
    while !shim_data.is_empty() {
        ...
        if sig != sig_value {
            warn!("Did not get win8 signature. Got {sig}");
            break;
        }
...
mod tests {
    ...
    fn test_win81_format() {
        ...
        let (_, shim_data) = win8_format(&buffer, "test", "test/test").unwrap();
        ...
        assert_eq!(shim_data[0].last_modified, "2021-05-18T07:41:53.435Z");
        ...
        assert_eq!(shim_data[2].last_modified, "2014-11-21T09:16:52.634Z");
