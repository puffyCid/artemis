...
pub(crate) async fn aws_upload(
    ...
    let setup = setup_upload(aws_info, aws_endpoint_url, &aws_filename, &HashMap::new()).await?;

    aws_start_upload(setup, data).await
}
...
pub(crate) async fn setup_upload(
    info: AwsInfo,
    url: Url,
    filename: &str,
    ...
    let response = aws_create_multipart(url.as_str(), headers).await?;
...
async fn aws_start_upload(setup: AwsSetup, output_data: &[u8]) -> Result<(), RemoteError> {
    let first_upload = 1;
...
async fn aws_create_multipart(
    ...
    while attempts < max_attempts {
        ...
        let session_result = builder.send().await;
        ...
        let res_result = session.text().await;
...
pub(crate) async fn aws_complete_multipart(
    bucket: &Bucket,
    creds: &Credentials,
    aws_filename: &str,
    upload_id: &str,
    ...
    while attempts < max_attempts {
        ...
        let complete_result = complete_builder.body(action.clone().body()).send().await;
...
pub(crate) async fn aws_multipart_upload(
    output_data: &[u8],
    upload_id: &str,
    bucket: &Bucket,
    creds: &Credentials,
    ...
    while attempts < max_attempts {
        ...
        let res_result = builder.body(output_data.to_vec()).send().await;
        let response = match res_result {
            Ok(result) => result,
            Err(err) => {
...
mod tests {
    ...
    async fn test_aws_upload() {
        ...
        aws_upload(test.as_bytes(), &output, name).await.unwrap();
        mock_me.assert_hits(2);
        mock_me_put.assert();
    }

    #[tokio::test]
    async fn test_setup_upload() {
        let server = MockServer::start();
        let port = server.port();
        let output = output_options("aws_upload_test", "aws", "tmp", false, port);
    ...
    #[tokio::test]
    async fn test_aws_upload_compress() {
        let server = MockServer::start();
        let port = server.port();
    ...
    #[tokio::test]
    async fn test_aws_start_upload() {
        let server = MockServer::start();
        let port = server.port();
        let output = output_options("aws_upload_test", "aws", "tmp", false, port);
        ...
        let setup = setup_upload(aws_info, url, name, &HashMap::new())
            .await
            .unwrap();

        aws_start_upload(setup, test.as_bytes()).await.unwrap();
        mock_me.assert_hits(2);
        mock_me_put.assert();
    }

    #[tokio::test]
    async fn test_aws_create_multipart() {
        let server = MockServer::start();
        let port = server.port();
        ...
        let result = aws_create_multipart(&url, &HashMap::new()).await.unwrap();
        assert_eq!(result, "hi");
        mock_me.assert();
    }

    #[tokio::test]
    async fn test_aws_multipart_upload() {
        let server = MockServer::start();
        let port = server.port();
    ...
    #[tokio::test]
    async fn test_aws_complete_multipart() {
        let server = MockServer::start();
        let port = server.port();
        ...
        let bucket = Bucket::new(url, UrlStyle::VirtualHost, "blah", "us-east-1").unwrap();
        aws_complete_multipart(&bucket, &creds, name, "myid", Vec::new())
            .await
            .unwrap();
        mock_me.assert();
    }
