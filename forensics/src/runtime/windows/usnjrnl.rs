use crate::{
    artifacts::os::windows::usnjrnl::parser::grab_usnjrnl_entries,
...
pub(crate) fn js_usnjrnl(
    ...
    let jrnl = match grab_usnjrnl_entries(&options) {
        Ok(result) => result,
        Err(err) => {
...
mod tests {
    ...
    fn test_js_usnjrnl_rs_files() {
        ...
        let mut output = output_options("runtime_usnjrl_test", "./tmp", false);
        let script = JSScript {
