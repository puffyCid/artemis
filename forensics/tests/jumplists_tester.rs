...
fn test_jumplists_parser() {
    ...
    for result in results {
        ...
        if output_file.contains("\\jumplists_") && output_file.ends_with(".jsonl") {
...
fn validate_output(output: &PathBuf) {
    ...
    for (_, line) in reader.lines().enumerate() {
        ...
        let info: JumplistEntry = serde_json::from_str(&value).unwrap();
        if info.app_id.is_empty() {
            println!("{value}");
            panic!("no appID?")
        }
        assert_ne!(info.jumplist_metadata.modified, "1970-01-01T00:00:00.000Z");
    }
}
