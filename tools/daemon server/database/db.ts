...
export class LocalSqlite {
     ...
     */
    public newCollectionScript(payload: string, collection_id: number, collection_timeout: number): void {
        const insert = this.db.prepare(`INSERT INTO collection_scripts (collection_id, script, collection_timeout) VALUES (?, ?, ?)`);
        insert.run(collection_id, payload, collection_timeout);
    }
     ...
     */
    public getCollections(endpoint_id: string): { script: string, collection_id: number; collection_timeout: number; } | undefined {
        const query = this.db.prepare(`SELECT script, collections.collection_id AS collection_id, collection_timeout FROM collections INNER JOIN collection_scripts where endpoint_id = ? AND status = 'Not Started' AND collection_scripts.collection_id = collections.collection_id`);
        ...
        const collection_timeout = status[ "collection_timeout" ];
        if (typeof script === 'string' && typeof collection_id === 'number' && typeof collection_timeout === 'number') {
            return { script, collection_id, collection_timeout };
        }
    ...
    private setupTables(): void {
        try {
            ...
            const collection_scripts = `CREATE TABLE collection_scripts(id PRIMARY KEY, collection_id INTEGER, script TEXT, collection_timeout INTEGER)`;
